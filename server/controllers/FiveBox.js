const FiveBox = require('../models/FiveBox');

exports.createFiveBox = async (req, res) => {
  try {
    const {
...
exports.getAllFiveBoxes = async (req, res) => {
  try {
    const boxes = await FiveBox.find();
...
exports.getFiveBoxByRef = async (req, res) => {
  try {
    const box = await FiveBox.findOne({ reference: req.params.ref });
    if (!box) {
...
exports.updateFiveBox = async (req, res) => {
  try {
    ...
    const updatedBox = await FiveBox.findOneAndUpdate(
      { reference: ref },
      updateData,
      { new: true }
    );
...
exports.deleteFiveBox = async (req, res) => {
  try {
    const { ref } = req.params;
    const deletedBox = await FiveBox.findOneAndDelete({ reference: ref });
    if (!deletedBox) {
      return res.status(404).json({ error: 'Produit non trouvé' });
    }
...
exports.deleteAllFiveBoxes = async (req, res) => {
  try {
    await FiveBox.deleteMany({});
