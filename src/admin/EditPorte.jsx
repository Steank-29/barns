...
const EditPorte = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/porte/getallportes');
        if (!response.ok) {
  ...
  return (
    <div style={{ padding: isMobile ? '10px' : '20px' }}>
      <Typography variant={isMobile ? 'h5' : 'h4'} gutterBottom sx={{ 
        ...
        Portes Management – Solutions Intelligentes de Gestion et de Suivi
      </Typography>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Box display="flex" alignItems="center">
              ...
              <Typography variant={isMobile ? 'h6' : 'h6'} fontWeight={600} sx={{ letterSpacing: '0.5px' }}>
                Gérer et Modifier les Détails de Porte
              </Typography>
            </Box>
            <IconButton 
              onClick={closeDialog} 
