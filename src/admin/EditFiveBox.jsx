...
const buttonStyles = (variant) => ({
  ...
  useEffect(() => {
    const fetchThreeBoxes = async () => {
      try {
        const response = await fetch('https://barns.onrender.com/api/fivebox/getallfiveboxes');
        if (!response.ok) {
...
const handleEdit = (threeBox) => {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`https://barns.onrender.com/api/fivebox/deletefivebox/${threeBoxToDelete.reference}`, {
        method: 'DELETE'
      });
...
const saveChanges = async () => {
  try {
    ...
    const response = await fetch(`https://barns.onrender.com/api/fivebox/updatefivebox/${selectedThreeBox.reference}`, {
      method: 'PUT',
      body: formData,
