...
const saveChanges = async () => {
  try {
    ...
    if (response.ok) {
      ...
      setSnackbar({
        open: true,
        message: 'TwoBox mise à jour avec succès',
        severity: 'success'
      });
      setEditMode(false);
      window.location.reload();
    } else {
      throw new Error('Échec de la mise à jour de la TwoBox');
    }
  } catch (error) {
    console.error('Erreur lors de la mise à jour de la TwoBox:', error);
    setSnackbar({
      open: true,
      message: 'Erreur lors de la mise à jour de la TwoBox',
      severity: 'error'
    });
  }
};
