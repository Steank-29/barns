...
const [selectedProduct, setSelectedProduct] = useState({
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/malle/getallmalles');
        if (!response.ok) {
...
const handleEdit = (product) => {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/malle/deletemalle/${productToDelete.reference}`, {
        method: 'DELETE'
      });
...
const saveChanges = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/malle/updatemalle/${selectedProduct.reference}`, {
      method: 'PUT',
      body: formData,  
    });
    ...
    if (response.ok) {
      ...
      setSnackbar({
        open: true,
        message: 'Malle mise à jour avec succès',
        severity: 'success'
      });
      setEditMode(false);
      window.location.reload();
    } else {
      throw new Error('Échec de la mise à jour de la malle');
    }
  } catch (error) {
    console.error('Erreur lors de la mise à jour de la malle:', error);
    setSnackbar({
      open: true,
      message: 'Erreur lors de la mise à jour de la malle',
      severity: 'error'
    });
  }
};
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Box display="flex" alignItems="center">
              ...
              <Typography variant={isMobile ? 'h6' : 'h6'} fontWeight={600} sx={{ letterSpacing: '0.5px' }}>
                Gérer et Modifier les Détails de Malle
              </Typography>
            </Box>
            <IconButton 
              onClick={closeDialog} 
