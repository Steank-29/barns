...
const getValidImageUrl = (imageUrl) => {
  if (!imageUrl) return "/placeholder-twoboxresin.jpg";
...
export default function TwoBoxResinPage() {
  ...
  const activeItem = twoBoxResins.find((item) => item._id === selectedId);
  const description =
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 5 }}>
      ...
      >
        Notre Collection de 2 Box Résine
      </Typography>
          ...
          <ProductCard elevation={3}>
            ...
            <SelectionMenuBox>
              ...
              <FormControl size="small">
                <InputLabel
                  id="twoboxresin-select-label"
                  sx={{ color: "#38598b", fontFamily: "Savate" }}
                >
                  2 Box Résine
                </InputLabel>
                <Select
                  labelId="twoboxresin-select-label"
                  value={selectedId}
                  onChange={handleSelectChange}
                  sx={{ minWidth: 160, fontFamily: "Savate" }}
                >
                  {twoBoxResins.map((item) => (
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", my: 2 }}>
                ...
                {activeItem.longueur && (
                  <Chip label={`Longueur: ${activeItem.longueur}`} size="small" sx={{ backgroundColor: "grey.100", fontFamily: "Savate" }} />
                )}
                {activeItem.profondeur && (
                  <Chip label={`Profondeur: ${activeItem.profondeur}`} size="small" sx={{ backgroundColor: "grey.100", fontFamily: "Savate" }} />
                )}
                {activeItem.pannes && (
                  <Chip label={`Pannes: ${activeItem.pannes}`} size="small" sx={{ backgroundColor: "grey.100", fontFamily: "Savate" }} />
                )}
                {activeItem.ossatureM && (
                  <Chip label={`Ossature M: ${activeItem.ossatureM}`} size="small" sx={{ backgroundColor: "grey.100", fontFamily: "Savate" }} />
                )}
              </Box>
